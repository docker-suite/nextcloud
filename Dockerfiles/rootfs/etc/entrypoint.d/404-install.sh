...
if version_greater "$image_version" "$installed_version"; then
    ...
    rsync $rsync_options --delete --exclude "/config/" --exclude "/custom_apps/" --exclude "/themes/" "$NEXTCLOUD_INSTALL_DIR/" "$NEXTCLOUD_WWW_DIR/"

    # Update config, appstore and themes folder if they are empty
...
mvlink "$NEXTCLOUD_WWW_DIR/themes" "$NEXTCLOUD_THEMES_DIR"
